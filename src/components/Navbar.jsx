    ...
    <>
            ...
            <div className="hidden md:flex items-center gap-6 lg:gap-8">
              ...
              {user ? (
                <div className="relative" ref={accountRef}>
                  <button onClick={() => setAccountOpen(!accountOpen)} aria-haspopup="true" aria-expanded={accountOpen} className="flex items-center gap-3 focus:outline-none">
                    <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-orange-500">
                      <FaUserCircle className="w-8 h-8" />
                    </div>
                    <div className="hidden lg:block text-sm text-gray-700">Hi, <span className="font-semibold">{user.name || (user.email ? user.email.split('@')[0] : 'User')}</span></div>
                  </button>

                  {accountOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg p-3 z-50">
                      <div className="truncate text-sm font-semibold text-gray-800">{user.name || (user.email || 'User')}</div>
                      <div className="truncate text-xs text-gray-500 mb-3">{user.email}</div>
                      <NavLink to="/personalize" onClick={() => setAccountOpen(false)} className="block w-full text-center px-3 py-2 bg-orange-500 text-white rounded-md mb-2 text-sm">Dashboard</NavLink>
                      <button onClick={() => { onLogout(); }} className="w-full px-3 py-2 bg-red-500 text-white rounded-md text-sm">Logout</button>
                    </div>
                  )}
                </div>
              ) : (
      ...
      <div className={`md:hidden fixed left-0 w-full z-50 transition-all bg-white duration-300 ${mobileMenuOpen ? 'top-[90px] opacity-100 pointer-events-auto' : '-top-96 opacity-0 pointer-events-none'}`}>
        <nav className="flex flex-col gap-2 p-4 max-h-[70vh] overflow-y-auto">
          <NavLink to="/" onClick={() => setMobileMenuOpen(false)} className={({isActive}) => `block w-full text-left px-4 py-3 rounded-lg text-base font-medium transition ${isActive ? 'bg-green-400 text-white' : 'text-gray-800 hover:bg-gray-50'}`}>Home</NavLink>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100">
            {user ? (
              <NavLink to="/personalize" onClick={() => setMobileMenuOpen(false)} className="w-full block text-center px-4 py-3 bg-orange-500 text-white rounded-lg font-bold shadow text-lg">Dashboard</NavLink>
            ) : (
              <NavLink to="/auth" onClick={() => setMobileMenuOpen(false)} className="w-full block text-center px-4 py-3 bg-orange-500 text-white rounded-lg font-bold shadow text-lg">Get Started</NavLink>
            )}
          </div>
          ...
          {user && (
            <div className="mt-3">
              <button onClick={onLogout} className="w-full block text-center px-4 py-3 bg-red-500 text-white rounded-lg font-bold shadow">Logout</button>
            </div>
          )}
        </nav>
      </div>
